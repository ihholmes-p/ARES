        ...
        for episode_length in episode_lengths:
            ...
            for i in range(num_trials):
                for reward_file in reward_files:
                    inferred_trial, avg_scores = run_ll(infile=reward_file, reward_type="delayed", epsilon_start=epsilon_start, num_episodes=episode_length, num_to_solve=num_to_solve)
                    if (inferred_trial == True):
                        inferred_optimals += 1
                    # if (episode_length == 1000):
                    #     _, avg_scores = run_ll(infile=reward_file, reward_type="delayed", epsilon_start=epsilon_start, num_episodes=1000, num_to_solve=2000)
