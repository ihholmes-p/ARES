...
def exp_gen(epsilon_starts, episode_lengths, num_to_solve, outfile):
    ...
    reward_files = [
        "ll_random_rewards1.txt",
        "ll_random_rewards2.txt",
        "ll_random_rewards3.txt",
        "ll_random_rewards4.txt",
        "ll_random_rewards5.txt",
        "ll_random_rewards6.txt",
        "ll_random_rewards7.txt",
        "ll_random_rewards8.txt",
        "ll_random_rewards9.txt",
        "ll_random_rewards10.txt",
    ]
        ...
        for episode_length in episode_lengths:
            ...
            for i in range(num_trials):
                for reward_file in reward_files:
                    inferred_trial, avg_scores = run_ll(infile=reward_file, reward_type="immediate", epsilon_start=epsilon_start, num_episodes=episode_length, num_to_solve=num_to_solve)
                    if (inferred_trial == True):
                        inferred_optimals += 1
                    # if (episode_length == 1000):
                    #     _, avg_scores = run_ll(infile=reward_file, reward_type="immediate", epsilon_start=epsilon_start, num_episodes=1000, num_to_solve=2000)
